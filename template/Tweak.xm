...
// 1. درع التخفي لمنع اكتشاف الحقن
...
// 2. إخفاء ملفات الهاك من اللعبة (نسخة C++ الخام)
...
int custom_stat(const char *restrict path, struct stat *restrict buf) {
    if (path != NULL) {
        if (strstr(path, "ESign") || strstr(path, "dylib") || 
            strstr(path, "MobileSubstrate") || strstr(path, "ShadowTrackerExtra")) {
            return -1; 
        }
    }
